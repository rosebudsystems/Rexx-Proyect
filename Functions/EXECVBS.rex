/**********************************************************************/
/*                  Ejecuta un VisualBasicScript (*.vbs)              */
...
/**********************************************************************/
/*Imp1.0 Version Inicial - Se copia la Funcion BORRAX.rex como planitilla */
/*Imp1.0.1 Se agregan las modificaciones para la ejecucion del VBScript*/
...
!arch = STRREPLACE(!arch, '"','') 

call 0050_Check_Existencia
if rc\=0 then do
	!log_linea='Error Nro. 3505 : Archivo *.VBS no encontrado '||!arch
	...
	!arch = '"' || !arch  || '"'  /**/
	!ejecutar= 'C:\windows\System32\cscript.exe ' || !arch 
	/*!ejecutar= 'C:\windows\SysWow64\cscript.exe ' || !arch  */
	!log_linea= 'Linea de ejecucion: ' || !ejecutar /*Imp1.0.1 Se imprime la linea de ejecucion*/
	call 0022_Logging_Write
	
	!ejecutar /*Imp1.0.1 Se realiza la ejecucion*/
	if rc\=0 then do		
			!log_linea='Error Nro. 3619 : Error al ejecutar el Vbscript. ('||rc||')' 
			call 0022_Logging_Write
			!log_file~close
			Exit 3609  /*Imp1.0.1 Se cambia el mensaje de error*/
	end
end
!log_file~close
